...
FLAGS_FIRST_RUN = False
FLAGS_CORPUS_NAME = "wiki"
folder = '../../data/'+FLAGS_CORPUS_NAME+'/intermediate/'
...
def main():
  ...
  if FLAGS_FIRST_RUN:
    ...
    start = time.time()
    skipgram2id, skipgram2eidcounts, eid2skipgramcounts = util.load_skipgram2eidcounts(folder + 'eidSkipgramCounts.txt')
    end = time.time()
  ...
  eid2types = util.loadEidToTypeMap(folder + 'linked_results.txt', ename2eid=ename2eid)
  ...
  start = time.time()
  expanded_eids = setExpan(es, seedEidsWithConfidence, negativeSeedEids, eid2ename, eid2types, params, FLAGS_DEBUG=False)
  end = time.time()
