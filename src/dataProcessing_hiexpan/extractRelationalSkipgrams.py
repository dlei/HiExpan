...
def getRelationalSkipgrams(tokens, start1, end1, start2, end2, window=5):
  ...
  for pos in positions:
    if start1 < start2:
      relational_skipgrams.append(' '.join(tokens[max(start1+pos[0],0):start1])+' EM1 '+' '.join(tokens[end1+1:start2])+' EM2 '+' '.join(tokens[end2+1:min(end2+1+pos[1], max_len)]))
  else:
    relational_skipgrams.append(' '.join(tokens[max(start2+pos[0],0):start2])+' EM2 '+' '.join(tokens[end2+1:start1])+' EM1 '+' '.join(tokens[end1+1:min(end1+1+pos[1], max_len)]))
...
if __name__ == "__main__":
  ...
  inputFileName = "../../data/"+corpusName+"/intermediate/sentences.json"
