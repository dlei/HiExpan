...
__description__: A utility class
...
class TreeNode:
  ...
  def __str__(self):
    if not self.parent:
      ...
      # return "TreeNode (level=%s, ename=%s, eid=%s, confidence_score=%s) under parent (level=%s, ename=%s, eid=%s, confidence_score=%s)" % \
      #        (self.level, self.ename, self.eid, self.confidence_score, self.parent.level, self.parent.ename, self.parent.eid, self.parent.confidence_score)
      return ("%s (eid=%s,log_prob=%0.6f,parent=%s)" % (self.ename, self.eid, self.confidence_score, self.parent.ename) )
  ...
  def cutFromChild(self, child):
    ...
    '''
    if self.children == None:
      # raise Exception("[WARNING] Unable to cut from child in empty TreeNode object")
      ## this is possible because the father is being deleted during previous around's pruning
      return
  ...
  def printSubtree(self, tabs):
    for i in range(tabs):
      print('\t', end="")
    print(self.ename, " (eid=%s, log_prob=%0.6f)" % (self.eid, self.confidence_score))
  ...
  def saveToFile(self, outputFilePath):
    with open(outputFilePath,"w") as fout:
      ...
      while(len(s) != 0):
        ...
        fout.write(tab_number * "\t" + entity_name + " (eid=%s, log_probability=%0.6f)" % (top.eid, top.confidence_score) + "\n")
