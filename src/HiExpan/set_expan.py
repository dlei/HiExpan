...
__description__: Python Implementation of SetExpan algorithm
    TO Ellen: I change the interface of setExpan() to avoid the usage of TreeNode, so that this file can be
    treated as an individual package later and also easier for testing.

__latest_update__: 02/02/2018
'''
import random
import math
import time
...
TOP_K_SG = 200
# TOP_K_SG is the maximum number of types that wil be selected to calculate the entity-entity
# distributional similarity.
TOP_K_TYPE = 50
...
def getDominantType(eid, eid2types, eidAndType2strength):
  #TODO: Change other way to use type information
  dominantType = None
  strength = -999
...
def sim_sib(eid1, eid2, alpha, eid2patterns, pattern2eids, eidAndPattern2strength, eid2embed, eid2types,
  ...
  skipgram_sim = getFeatureSim(eid1, eid2, eidAndPattern2strength, quality_skipgram_features)
  if skipgram_sim < 0:
    skipgram_sim = 0
  ...
  overall_sim = embedding_sim * math.exp(skipgram_sim) * math.exp(type_sim)
...
def setExpan(seedEidsWithConfidence, negativeSeedEids, eid2patterns, pattern2eids, eidAndPattern2strength,
  ...
  while iters < MAX_ITER_SET:
    ...
    if use_type:
      ...
      ## use all the types, no type selection because the type information is already sparse
    ...
    for ele in sorted(eid2aggregated_mrr.items(), key=lambda x:-x[1])[0:MAXIMUM_EXPAND]:
      ...
      if eid not in negativeSeedEids: # directly filter negativeSeedEids
        ...
        for prev_eid in seedEids:
          ## New method:
          sibling_similarity = sim_sib(eid, prev_eid, 0.5, eid2patterns, pattern2eids, eidAndPattern2strength,
                                       eid2embed, eid2types, eidAndType2strength, topK_quality_sg=TOP_K_SG, DEBUG=False)
          # sibling_similarity = sim_sib_embed_only(eid, prev_eid, eid2embed)
        ...
        # print("sib_sim_list", sib_sim_list)
        ...
        if len(sib_sim_list) < 3:
          ...
          # # Old method
          # sibling_similarity = (aggregated_mrr / max_aggregated_mrr)
          # confidence_score += (eid2confidence[prev_eid] + math.log(sibling_similarity))
...
def getCoherenceScore(eids, negativeSeedEids, eid2patterns, pattern2eids, eidAndPattern2strength,
             eid2types, type2eids, eidAndType2strength, eid2ename, eid2embed, alpha=0.5,
                      eid2FLAGS_VERBOSE=False, FLAGS_DEBUG=False):
  combinedWeightBySkipgramMap = getCombinedWeightByFeatureMap(eids, eid2patterns, eidAndPattern2strength)
  redundantSkipgrams = set()
  for i in combinedWeightBySkipgramMap:
    size = len(pattern2eids[i])
    if size < FLAGS_SG_POPULARITY_LOWER or size > FLAGS_SG_POPULARITY_UPPER:
  ...
  for sg in redundantSkipgrams:
    del combinedWeightBySkipgramMap[sg]
  print("[INFO] Total number of skipgrams = %s" % len(combinedWeightBySkipgramMap))

  ## select only top-K quality skipgrams
  quality_combinedWeightBySkipgramMap = {}
  topK_quality_sg = 1000
  for ele in sorted(combinedWeightBySkipgramMap.items(), key=lambda x: -x[1])[0:topK_quality_sg]:
    quality_combinedWeightBySkipgramMap[ele[0]] = ele[1]
  ...
  for i, eid1 in enumerate(eids):
    ...
    for j, eid2 in enumerate(eids):
      if j == i:
        ...
        sim_vector.append(sim_matrix[j][i])
      else:
        # sim_vector.append(getFeatureSim(eid1, eid2, eidAndPattern2strength, combinedWeightBySkipgramMap))
        embedding_sim = float(1.0 - distance.cdist(eid2embed[eid1], eid2embed[eid2], 'cosine'))
        skipgram_sim = getFeatureSim(eid1, eid2, eidAndPattern2strength, quality_combinedWeightBySkipgramMap)
        overall_sim = (embedding_sim ** alpha) * (skipgram_sim ** (1-alpha))
        sim_vector.append(overall_sim)
    sim_matrix.append(sim_vector)

  eids_coherence = []
  for i in range(len(eids)):
    if len(eids) == 1:
      coherence = 1 # with itself
    else:
      coherence = ( sum(sim_matrix[i]) - 1 ) / (len(eids)-1)
    eids_coherence.append(coherence)
  return eids_coherence
